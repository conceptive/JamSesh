class ApplicationController < ActionController::Base
  ...
  def after_update_path_for(resource)
    ...
    devise_parameter_sanitizer.for(:account_update) << :username << :name << :email << :password << :password_confirmation << :avatar << :location << :bio << :styles << :jam_skill_id << :user_instruments << :main_instrument
	end
end
